...
export async function searchMemory(cfg, payload) {
  ...
  const minSimilarity = cfg.minSimilarity ?? 0.5;
  const timeDecayAlpha = cfg.timeDecayAlpha ?? 0.09;

  // Time decay formula (applied AFTER fetching, in the .map() below):
  //   final_score = cosine_similarity × 1/(1 + α × ln(1 + days_old))
  // When α = 0, decay is disabled (multiplier = 1.0 always).
  //
  // Why over-fetch (3x)?
  //   Decay is calculated after retrieval, not in the SQL query.
  //   Some results may drop below minSimilarity after decay is applied.
  //   We fetch extra candidates so that after filtering we still have
  //   enough results to return (up to halfLimit).
  //   Flow: DB fetch (3x) → apply decay → filter by threshold → sort → take top N
  const fetchLimit = timeDecayAlpha > 0 ? halfLimit * 3 : halfLimit;

  // Search chat_messages and vault_notes in parallel
  const [chatResult, notesResult] = await Promise.all([
    p.query(
      `SELECT id, content, role, session_id, timestamp, COALESCE(weight, 1.0) AS weight,
    ...
    p.query(
      `SELECT id, path, title, content, tags, file_modified_at, COALESCE(weight, 1.0) AS weight,
              1 - (embedding <=> $1::vector) AS similarity
       FROM vault_notes
       WHERE embedding IS NOT NULL
       ORDER BY embedding <=> $1::vector
  ...
  // Apply time decay and weight boost to chat_messages, then filter by threshold.
  // Formula: final_score = cosine_similarity × time_decay × weight
  // - weight=1.0 (default): normal memory
  // - weight=2.0: pinned/important memory (effectively doubles its score)
  // - weight=0: silently suppressed (will never pass threshold)
  const now = Date.now();
  const conversations = chatResult.rows
    .map((r) => {
      const sim = parseFloat(r.similarity);
      const weight = parseFloat(r.weight);
      let finalScore = sim * weight;
      if (timeDecayAlpha > 0 && r.timestamp) {
        const ageMs = now - new Date(r.timestamp).getTime();
        const ageDays = Math.max(0, ageMs / 86400000);
        const decay = 1.0 / (1.0 + timeDecayAlpha * Math.log(1.0 + ageDays));
        finalScore = sim * decay * weight;
      }
      return {
        id: r.id,
        content: r.content,
  ...
  // vault_notes: no time decay, but weight is applied
...
function stripEnvelope(text) {
  ...
  // Remove [[reply_to_current]] and [[reply_to:<id>]] tags
  text = text.replace(/\[\[\s*reply_to_current\s*\]\]\s*/g, "");
  text = text.replace(/\[\[\s*reply_to:\s*\S+?\s*\]\]\s*/g, "");
  // Collapse multiple blank lines into one, then trim leading/trailing whitespace
  text = text.replace(/\n{3,}/g, "\n\n");
  return text.trim();
}
...
export function buildConfig(pluginConfig = {}) {
  ...
  return {
    ...
    minSimilarity: cfg.minSimilarity ?? 0.5,
    timeDecayAlpha: cfg.timeDecayAlpha ?? 0.09,
