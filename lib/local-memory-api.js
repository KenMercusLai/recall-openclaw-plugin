...
export async function addMessage(cfg, payload) {
  ...
  const insertResult = await p.query(
    `INSERT INTO chat_messages (content, role, session_id, session_label, "timestamp", created_at, metadata)
     VALUES ($1, $2, $3, $4, COALESCE($6::timestamptz, NOW()), NOW(), $5::jsonb)
     RETURNING id`,
    [content, role || "user", sessionId || "unknown", sessionLabel, metadataJson, msgTimestamp],
  );
