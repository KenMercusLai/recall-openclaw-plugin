...
// Resolve pg from workspace node_modules or local
let pg;
try {
  pg = await import("pg");
} catch {
...
export function getPool(cfg) {
  ...
  pool.on("error", (err) => {
    console.warn("[memos-local] Pool error:", err.message);
  });
  return pool;
}
...
export async function addMessage(cfg, payload) {
  const { sessionKey, role, content } = payload;
  if (!content) return;
  ...
  const insertResult = await p.query(
    `INSERT INTO chat_messages (content, role, session_id, "timestamp", created_at)
     VALUES ($1, $2, $3, NOW(), NOW())
     RETURNING id`,
    [content, role || "user", sessionKey || "unknown"],
  );
  ...
  try {
    ...
    console.warn(`[memos-local] Failed to generate embedding for message ${messageId}:`, err.message);
  }
}
