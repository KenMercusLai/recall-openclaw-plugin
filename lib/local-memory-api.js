...
export async function addMessage(cfg, payload) {
  const { sessionKey, role, content, metadata = {} } = payload;
  if (!content) return;
  ...
  const insertResult = await p.query(
    `INSERT INTO chat_messages (content, role, session_id, "timestamp", created_at, metadata)
     VALUES ($1, $2, $3, NOW(), NOW(), $4::jsonb)
     RETURNING id`,
    [content, role || "user", sessionKey || "unknown", metadataJson],
  );
